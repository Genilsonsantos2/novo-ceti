...
export const StudentsPage: React.FC = () => {
  ...
  // Hidden Migration Script
  const handleMigratePhotos = async () => {
    if (!window.confirm('Tem certeza que deseja migrar as fotos Base64 para o Supabase Storage?')) return;
    setIsMigrating(true);
    let count = 0;
    
    try {
      const { data: studentsWithBase64 } = await supabase
        .from('students')
        .select('id, photo_url')
        .like('photo_url', 'data:image%');

      if (!studentsWithBase64 || studentsWithBase64.length === 0) {
        alert('Nenhuma foto em Base64 encontrada para migrar.');
        setIsMigrating(false);
        return;
      }

      console.log(`Encontrados ${studentsWithBase64.length} alunos com foto em Base64. Iniciando migração...`);
      ...
      for (const st of studentsWithBase64) {
        try {
          ...
          if (!uploadError) {
            ...
            count++;
            console.log(`Migrado aluno ${st.id} (${count}/${studentsWithBase64.length})`);
          }
        } catch (e) {
          console.error(`Erro ao migrar foto do aluno ${st.id}:`, e);
        }
      }
      alert(`Migração concluída! ${count} fotos migradas.`);
      fetchStudents();
    } catch (err) {
      console.error('Erro geral na migração:', err);
      alert('Ocorreu um erro ao buscar alunos para migração.');
    }
    setIsMigrating(false);
  };
  ...
  const handleCreateStudent = async (e: React.FormEvent) => {
    ...
    try {
      // Upload photo if it's base64 (newly uploaded)
      ...
      if (photoUrl.startsWith('data:image')) {
        try {
          ...
          console.error("Failed to upload photo to storage:", e);
        }
      }
  ...
  return (
    <div className="flex-1 px-6 md:px-10 py-8 min-h-screen relative">
      <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <p className="text-xs font-bold text-primary uppercase tracking-widest mb-2 opacity-70" onDoubleClick={handleMigratePhotos}>
            ...
            return (
              <div ref={parentRef} className="max-h-[60vh] overflow-auto relative">
                <table className="w-full text-left">
                  ...
                  {loading ? (
                    <tbody><tr><td colSpan={6} className="px-8 py-16 text-center">
